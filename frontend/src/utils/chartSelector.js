...
const NUMERIC_GUESS = (val) => typeof val === "number" || (typeof val === "string" && /^-?\d+(\.\d+)?$/.test(val));
export function selectChartType(response) {
    ...
    const numericCols = columns.filter((_, i) => rows.slice(0, 5).every((r) => r[i] == null || NUMERIC_GUESS(r[i])));
...
export function getChartAxes(response) {
    const { columns, rows } = response;
