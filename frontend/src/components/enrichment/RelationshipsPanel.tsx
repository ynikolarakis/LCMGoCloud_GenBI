...
export function RelationshipsPanel({
  ...
  const [leftCol, setLeftCol] = useState<ColumnInfo | null>(null);
  const [rightCol, setRightCol] = useState<ColumnInfo | null>(null);
  const [newType, setNewType] = useState("many-to-one");
  const [newDesc, setNewDesc] = useState("");

  const leftTableObj = tables.find(
    (t) => `${t.schema_name}.${t.table_name}` === leftTable,
  );
  const rightTableObj = tables.find(
    (t) => `${t.schema_name}.${t.table_name}` === rightTable,
  );
  ...
  const createMut = useMutation({
    mutationFn: (data: RelationshipCreate) =>
      createRelationship(connectionId, data),
    onSuccess: () => {
      invalidateSchema();
      setLeftCol(null);
      setRightCol(null);
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, ...data }: { id: string; relationship_type?: string; description?: string }) =>
  ...
  const handleCreate = () => {
    if (!leftCol || !rightCol || !leftTableObj || !rightTableObj) return;
    createMut.mutate({
  ...
  return (
    <div className="mt-6 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      ...
      <button
        ...
        className="flex w-full items-center justify-between border-b border-gray-100 bg-gray-50/50 px-5 py-3"
      >
        ...
        <div className="p-5">
          ...
          {relationships.length > 0 ? (
            <div className="mb-5 space-y-1.5">
              {relationships.map((rel) => (
                <div
                  key={rel.id}
                  className="flex items-center gap-2 rounded-lg border border-gray-100 bg-gray-50/50 px-3 py-2 text-xs"
                >
                  {editingId === rel.id ? (
                    <>
                      ...
                      <select
                        ...
                        className="ml-2 rounded border border-gray-200 px-1.5 py-0.5 text-[11px]"
                      >
                        {REL_TYPES.map((t) => (
                      ...
                      <input
                        ...
                        className="ml-1 flex-1 rounded border border-gray-200 px-2 py-0.5 text-[11px]"
                      />
                      <button
                        onClick={() =>
                          updateMut.mutate({
                    ...
                    <>
                      ...
                      <span className="ml-auto flex-shrink-0 rounded bg-gray-200/60 px-1.5 py-0.5 text-[9px] font-medium text-gray-500">
                        {rel.relationship_type}
                      </span>
                      {rel.description && (
                        <span className="max-w-[200px] truncate text-[10px] text-gray-400" title={rel.description}>
                          {rel.description}
                        </span>
                      )}
                      {!rel.is_auto_detected && (
                        <span className="rounded bg-blue-50 px-1 py-0.5 text-[9px] font-medium text-blue-500">
                          manual
                        </span>
                      )}
                      <button
            ...
            <div className="mb-5 rounded-lg border border-dashed border-gray-200 bg-gray-50/30 px-4 py-3 text-center">
              <p className="text-xs text-gray-400">No relationships defined. Use the creator below to add joins.</p>
            </div>
          )}

          {/* Click-click join creator */}
          <div className="rounded-lg border border-gray-200 bg-gray-50/50 p-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {/* Left side */}
              <div>
                <label className="mb-1 block text-[10px] font-medium text-gray-500">From Table</label>
                <select
                  value={leftTable}
                  onChange={(e) => {
                ...
                >
                  ...
                  {tables.map((t) => (
                    <option key={t.id} value={`${t.schema_name}.${t.table_name}`}>
                ...
                {leftTableObj && (
                  <div className="max-h-40 space-y-0.5 overflow-y-auto">
                    {leftTableObj.columns.map((col) => (
                      <button
                        key={col.id}
                        onClick={() => setLeftCol(col)}
                      ...
                      >
                        ...
                        <span className="ml-auto text-[10px] text-gray-300">{col.data_type}</span>
                      </button>
                    ))}
                  </div>
                )}
              ...
              {/* Right side */}
              <div>
                <label className="mb-1 block text-[10px] font-medium text-gray-500">To Table</label>
                <select
                  value={rightTable}
                  onChange={(e) => {
                ...
                {rightTableObj && (
                  <div className="max-h-40 space-y-0.5 overflow-y-auto">
                    {rightTableObj.columns.map((col) => (
                      <button
                        key={col.id}
                        onClick={() => setRightCol(col)}
                        className={`flex w-full items-center gap-1.5 rounded px-2 py-1 text-left text-[11px] transition-colors ${
                          rightCol?.id === col.id
                            ? "bg-blue-100 text-blue-700 ring-1 ring-blue-300"
                            : "text-gray-600 hover:bg-gray-100"
            ...
            {/* Create form appears when both columns selected */}
            {showCreateForm && (
              <div className="mt-3 flex items-center gap-3 border-t border-gray-200 pt-3">
                <span className="text-[11px] font-mono text-gray-600">
                  {leftTableObj!.table_name}.{leftCol.column_name}
                </span>
                <svg className="h-3 w-3 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" />
                </svg>
                <span className="text-[11px] font-mono text-gray-600">
                  {rightTableObj!.table_name}.{rightCol.column_name}
                </span>
                <select
                  value={newType}
                ...
                <button
                  ...
                  className="rounded-lg bg-blue-600 px-3 py-1.5 text-[11px] font-semibold text-white shadow-sm hover:bg-blue-700 disabled:opacity-50"
                >
