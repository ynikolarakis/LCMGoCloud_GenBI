...
function fmtNum(v: unknown): string {
  if (typeof v === "number") return v.toLocaleString("el-GR");
  return String(v ?? "");
}
...
function parseDuration(val: unknown): number | null {
  ...
  if (dayMatch || hourMatch || minMatch) {
    if (dayMatch) days = parseFloat(dayMatch[1].replace(",", "."));
...
function isDurationColumn(rows: unknown[][], colIdx: number): boolean {
...
export function ChartView({ response, chartType }: Props) {
  ...
  const durationCols = new Set<number>();
  columns.forEach((_, i) => {
    if (i !== labelIdx && isDurationColumn(rows, i)) durationCols.add(i);
  });
  ...
  const numericKeys = columns.filter((_, i) =>
    i !== labelIdx && (
      rows.some((r) => typeof r[i] === "number") || durationCols.has(i)
    )
  );
  ...
  const formatLegend = (value: string) => displayName(value);
  ...
  const tooltipFmt = (value: number, name: string): [string, string] => {
    const colIdx = columns.indexOf(name);
    if (colIdx >= 0 && durationCols.has(colIdx)) return [fmtDuration(value), displayName(name)];
    ...
    return (
      <ResponsiveContainer width="100%" height={380}>
        <BarChart data={data} margin={{ top: 10, right: 30, bottom: 20, left: 20 }}>
          ...
          />
          <YAxis tick={{ fontSize: 11 }} tickFormatter={yAxisFmt} width={80} />
          <Tooltip
