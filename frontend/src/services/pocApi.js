...
// ─── Public (POC user) ────────────────────────────────────
export const authenticatePoc = (pocId, password) => pocClient
    .post(`/poc/${pocId}/auth`, { password })
    .then((r) => {
...
export async function pocQueryStream(pocId, body, callbacks) {
    const token = localStorage.getItem(`poc_token_${pocId}`);
    const response = await fetch(`/api/v1/poc/${pocId}/query/stream?token=${encodeURIComponent(token ?? "")}`, {
        method: "POST",
...
export const createPoc = (connectionId, data) => adminClient
    .post(`/connections/${connectionId}/poc`, data, {
    headers: { "Content-Type": "multipart/form-data" },
})
    .then((r) => r.data);
export const listPocs = () => adminClient.get("/poc/list").then((r) => r.data);
export const listPocsForConnection = (connectionId) => adminClient.get(`/connections/${connectionId}/poc`).then((r) => r.data);
export const deactivatePoc = (pocId) => adminClient.post(`/poc/${pocId}/deactivate`);
export const deletePoc = (pocId) => adminClient.delete(`/poc/${pocId}`);
export const getPocLogoUrl = (pocId) => `/api/v1/poc/${pocId}/logo`;
export const hasPocToken = (pocId) => !!localStorage.getItem(`poc_token_${pocId}`);
export const clearPocToken = (pocId) => localStorage.removeItem(`poc_token_${pocId}`);
