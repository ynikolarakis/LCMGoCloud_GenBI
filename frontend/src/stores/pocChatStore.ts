...
export interface PocChatMessage {
  id: string;
  role: "user" | "assistant";
  content: string;
...
export interface PocConversation {
  id: string;
  title: string;
  messages: PocChatMessage[];
  conversationId: string | null;
  timestamp: number;
}

interface PocChatState {
  messages: PocChatMessage[];
  conversationId: string | null;
  isLoading: boolean;
  connectionId: string | null;
  modelId: string;
  history: PocConversation[];
  activeConversationId: string | null;

  initialize: (connectionId: string, modelId: string) => void;
...
export const usePocChatStore = create<PocChatState>((set, get) => ({
  messages: [],
  conversationId: null,
  isLoading: false,
  connectionId: null,
    ...
    set((s) => {
      ...
      return {
        ...
        history: s.history.map((h) =>
          h.id === activeId ? { ...h, messages: newMessages } : h,
        ),
      };
    });
  ...
  addErrorMessage: (error) => {
    ...
    set((s) => {
      ...
      return {
        messages: newMessages,
        history: s.history.map((h) =>
          h.id === activeConversationId ? { ...h, messages: newMessages } : h,
        ),
      };
    });
  ...
  loadConversation: async (id) => {
    try {
      ...
      const messages: PocChatMessage[] = data.messages.map((m) => ({
        id: m.id,
        role: m.role as "user" | "assistant",
        content: m.content,
        response: m.response_data as unknown as QueryResponse | undefined,
  ...
  loadHistoryFromServer: async () => {
    ...
    try {
      ...
      const history: PocConversation[] = convs.map((c) => ({
        id: c.id,
        title: c.title,
        messages: [],
        conversationId: null,
