"""Lab Query Engine V2 — Research-based multi-stage architecture.

Implements proven methodologies from text-to-SQL research:
1. Semantic Schema Linking (Stage 1) — Find relevant tables using embeddings
2. Focused SQL Generation (Stage 2) — Generate SQL with only relevant context
3. Self-Correction Loop — Execute, get error, retry with feedback
4. Few-Shot Learning — Include similar verified queries as examples
5. Verified Query Storage — Save successful queries for future use

Based on:
- DIN-SQL: Decomposed In-Context Learning (+10% accuracy)
- PET-SQL: Two-stage with PreSQL linking (+4% accuracy)
- MAGIC: Self-correction guidelines (+8% accuracy)
- Few-shot with Query-CoT-SQL format (+6.4% accuracy)
"""
...
from src.services.lab.prompts import LAB_ANALYSIS_SYSTEM, LAB_ANALYSIS_USER
...
# Model map with cheaper model for schema linking
...
# System prompt for SQL generation (focused, with self-correction support)
SQL_GENERATION_SYSTEM_V2 = """You are an expert data analyst. Generate SQL queries from natural language questions.
...
class LabQueryEngineV2:
    """Research-based multi-stage query engine."""
    ...
    def __init__(
        self,
        max_tables: int = 8,
        min_similarity: float = 0.2,  # Lowered for cross-language support
    ...
    async def ask(
    ...
    ) -> tuple[QueryResponse | QueryError, LabV2Metrics]:
        """Process a question using multi-stage architecture.
        ...
        try:
            llm_text, in_tok, out_tok = await self._invoke_llm(
                SQL_GENERATION_SYSTEM_V2, user_prompt, resolved_model_id
            )
            ...
            logger.error("Lab V2 SQL generation failed: %s", exc)
            return QueryError(
        ...
        if isinstance(result, QueryError):
            ...
            # Record failure if this was a known query
        ...
        if result.rows:
            try:
                ...
                logger.warning("Lab V2 analysis failed: %s", exc)
    ...
    async def _build_focused_context(
        ...
        async with get_db() as conn:
            ...
            # Get full info for linked tables only
            ...
            for table in all_tables:
                ...
                for col in table.columns:
                    ...
                    # Include value descriptions (important for accuracy)
                    if values:
                        val_strs = []
                        for v in values[:10]:
    ...
    async def _execute_with_correction(
        ...
        Returns:
            Tuple of (result, final_sql, correction_count)
        """
        attempts = 0
        current_sql = sql
    ...
    async def _correct_sql(
        ...
        try:
            llm_text, in_tok, out_tok = await self._invoke_llm(
                SQL_GENERATION_SYSTEM_V2, user_prompt, model_id
            )
    ...
    async def _generate_analysis(
        ...
        row_lines = [
            ...
            for row in rows[:100]  # Limit rows for analysis
        ]
        rows_text = f"{header}\n{separator}\n" + "\n".join(row_lines)
        ...
        text, in_tok, out_tok = await self._invoke_llm(
            LAB_ANALYSIS_SYSTEM, user_prompt, model_id
        )
    ...
    def _parse_json_response(self, text: str) -> dict:
        ...
        logger.warning("Could not parse JSON from Lab V2 LLM response")
